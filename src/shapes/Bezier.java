...
public class Bezier implements ShapeType {
	private int[][] XY;
	private int[][] points;
	...
	@Override
	public void setToMetaPoints(MetaPoint[] MPs) {
		// TODO Auto-generated method stub
		points = new int[MPs.length][2];
		for(int i = 0; i < MPs.length; i++) {
			points[i][0] = MPs[i].getX();
			points[i][1] = MPs[i].getY();
		}
		map();
	}
	@Override
	public void draw(Graphics g, boolean finish) {
		// TODO Auto-generated method stub
		if (finish) {g.drawPolygon(XY[0], XY[1], XY[0].length);}
		else {g.drawPolyline(XY[0], XY[1], XY[0].length);}
	}
	@Override
